...
README에는 사이트 기능과 이용 방법만 두고, 이 문서에는 개발/운영/데이터 생성 관련 내용만 기록합니다.
...
2. 기본값은 `5문항씩 1요청`입니다.
3. 응답은 `examId + number` 기준으로 검증한 뒤 문항별 `answerExplanation`, `choiceExplanations`를 저장합니다.
...
- 페이지 정리 로직은 이제 실제 `건시스템 ...` 헤더 패턴일 때만 제거합니다.
- 현재는 확인된 깨진 PDF에 대해 본문 보정 로직으로 대응합니다.
...
TODO:
- PDF 추출 과정에서 선택지 번호(`①`~`④`)가 일부 유실되거나 합쳐지는 경우를 자동 탐지하고, 회차/문항 번호 기준 경고 로그를 남기도록 `build_exam_json.py` 보강 필요
- 현재는 확인된 깨진 PDF 텍스트에 대해 개별 보정 로직으로 대응 중이므로, 신규 PDF 추가 시 빌드 로그를 함께 확인해야 함
...
- `generated/exams.json`에 PDF `20회차`, 총 `1000문항`이 들어 있습니다.
- 해설 생성은 캐시 기반 증분 방식이며, 실행 시점에 따라 일부 문항만 해설이 채워진 상태일 수 있습니다.
