...
- 진행 중인 모의고사를 이어서 풀 수 있고, 학습 기록 페이지에서 모의고사 이력 확인과 오답 복습을 할 수 있습니다.
...
- 문제별 풀이 기록 저장
- 진행 중인 모의고사 이어풀기
- 학습 기록 페이지에서 전체 풀이 기록 확인
- 학습 기록 페이지에서 진행 중인 모의고사와 모의고사 이력 확인
- 모의고사 이력에서 틀린 문제 다시 복습
- 정답 공개 후 문제 `AI 해설 보기`
...
- 해설 노트 검색
- 기록 JSON 내보내기 / 가져오기
- 이전에 응시한 모의고사 회차 재선택 시 재응시 확인 안내
...
모의고사를 끝내면 점수, 과목별 정답 수, 오답 수를 바로 확인할 수 있습니다.

- 결과 화면의 `학습 기록 보기` 버튼으로 학습 기록 페이지로 이동 가능
- 학습 기록 페이지의 모의고사 이력에서 `틀린 문제 다시보기` 지원
- 이전에 응시한 회차를 다시 선택하면 재응시 여부를 한 번 더 확인

### 8. 학습 기록 페이지

학습 기록 페이지에서는 누적 통계와 모의고사 진행 상태를 함께 볼 수 있습니다.

- 진행 중인 모의고사 이어풀기 상태 확인
- 모의고사 이력을 최신순으로 탐색
- 기록 도구에서 JSON 내보내기 / 가져오기 지원
...
학습 기록 페이지의 `기록 도구`에서 현재 기록을 JSON으로 복사하거나 다른 기록을 가져올 수 있습니다.
...
- Study Log 페이지의 Data Tools 패널에서 `JSON 파일 저장`을 지원합니다.
- 긴 JSON 텍스트를 복붙하지 않아도 전체 학습 기록을 `.json` 파일로 내려받아 공유할 수 있습니다.
- 기존 `JSON 텍스트 붙여넣기` 병합 기능에 더해 `JSON 파일 업로드`로도 기록 병합이 가능합니다.
